<template>
  <div class="ranking-list">
    ...
    <div v-for="(user, index) in users" :key="index" class="ranking-item">
      <div class="table-cell" style = "position:relative;left:3em">{{ index + 1 }}</div>
  ...
  <div class="table-cell">{{ (user.totalDuration / user.targetDuration * 100).toFixed(0) }}%</div>
  <div class="table-cell">
    <div :class="{ 'status-circle': true, 'status-green': user.status === 1, 'status-red': user.status === 0 }"></div>
  </div>
    </div>
  </div>
</template>
...
const users = ref<User[]>([]);
  ...
  try {
    ...
    if (responseData.code === 200 && responseData.data && responseData.data.rows) {
